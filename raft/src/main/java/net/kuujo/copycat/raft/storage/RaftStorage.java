...
public interface RaftStorage extends AutoCloseable {
  ...
  /**
   * Returns the index of the next entry in the log.
   * <p>
   * The returned index is the index that will be assigned to the next entry created via {@link RaftStorage#createEntry()}.
   *
   * @return The index of the next entry in the log.
   * @throws IllegalStateException If the log is not open.
   */
  long nextIndex();
   ...
   * <p>
   * Users should ensure that the returned {@link RaftEntry} is closed once the write is complete. Closing the entry will
   * result in its contents being persisted to the log. Only a single {@link RaftEntry} instance may be open via the
   ...
   * <p>
   * If the index is less than {@code 1} or greater than {@link RaftStorage#lastIndex()} then this method will return
   ...
   * <p>
   * Note that only entries that have been {@link RaftStorage#commit(long) committed} are considered for compaction. Therefore,
   * entries where {@link RaftEntry#index()} is greater than the highest committed index will be live.
   *
   * @param index The index to check.
   ...
   * <p>
   * This method essentially advances the log's {@link RaftStorage#lastIndex()} without writing any entries at the interim
   ...
   * removed from the log, and the log's {@link RaftStorage#lastIndex()} will become equal to {@code index}. If entries
   * have been {@link RaftStorage#skip(long) skipped} then the skipped entries will be truncated as well.
   *
   ...
   * @throws IndexOutOfBoundsException If the given index is not within the bounds of the log.
   */
  RaftStorage truncate(long index);

  /**
   * Commits entries up to the given index.
   * <p>
   * The given commit {@code index} must be greater than the current {@code Loggable#commitIndex()}. When entries are
   * committed to the log they are made available for compaction. Entries that have been committed may not be removed
   * via {@link RaftStorage#truncate(long)}, but prior entries with the same keys will be deduplicated and effectively
   ...
   * {@code commitIndex} will be reset to {@code 0}.
   *
  ...
  /**
   * Marks tombstones up to the given index as freed for compaction.
   * <p>
   * In contrast to {@link RaftStorage#commit(long)} which makes entries available for compaction via deduplication, this
   * method allows tombstones created by, e.g. {@code log.createEntry(Foo.class, true)} to be permanently removed from
   * the log.
   *
