...
abstract class ActiveState extends PassiveState {
  ...
  private AppendResponse doCheckPreviousEntry(AppendRequest request) {
    ...
    if (entry.readTerm() != request.logTerm()) {
      LOGGER.warn("{} - Rejected {}: Request log term does not match local log term {} for the same entry", context.getLocalMember().id(), request, entry.readTerm());
      return AppendResponse.builder()
        .withStatus(Response.Status.OK)
        .withTerm(context.getTerm())
        .withSucceeded(false)
        ...
        if (context.log().containsIndex(entry.index())) {
          ...
          if (match != null) {
            if (entry.readTerm() != match.readTerm()) {
              ...
              LOGGER.debug("{} - Appended {} to log at index {}", context.getLocalMember().id(), entry, entry.index());
            }
          } else {
            context.log().truncate(entry.index() - 1);
            try (RaftEntry transfer = context.log().createEntry()) {
  ...
  private void doApplyCommits(long commitIndex) {
    ...
    if (commitIndex != 0 && !context.log().isEmpty()) {
      if (context.getCommitIndex() == 0 || commitIndex > context.getCommitIndex() || context.getCommitIndex() > context.getLastApplied()) {
        LOGGER.debug("{} - Applying {} commits", context.getLocalMember().id(), commitIndex - Math.max(context.getLastApplied(), context.log().firstIndex()));
  ...
  protected VoteResponse handleVote(VoteRequest request) {
    ...
    else if (request.candidate() == context.getLocalMember().id()) {
      ...
      if (!context.log().isEmpty()) {
        ...
        if (index != 0 && index >= lastIndex) {
          if (term >= entry.readTerm()) {
            ...
            LOGGER.debug("{} - Rejected {}: candidate's last log term ({}) is in conflict with local log ({})", context.getLocalMember().id(), request, term, entry.readTerm());
            return false;
          }
        } else {
