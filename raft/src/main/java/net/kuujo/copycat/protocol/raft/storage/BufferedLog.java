 ...
 * to compact {@link BufferedLog#commit(long) committed} entries with duplicate keys from the log, retaining only the most recent
 * instance of any given key. However, for performance reasons it cannot guarantee that duplicates do not exist.
 * <p>
 * To create a log, use the {@link BufferedLog#builder()}
 * <pre>
 *   {@code
 ...
 * {@link BufferedLog#createEntry()} will result in the entry being persisted to the log, while calling
 * {@link net.kuujo.copycat.protocol.raft.storage.RaftEntry#close()} on an entry acquired via {@link BufferedLog#getEntry(long)} will result in the entry being released back
 * to the internal entry pool.
 * <p>
 * The log only allows a single entry to be opened via {@link BufferedLog#createEntry()} at any given time, and it enforces
 * this restriction by throwing a {@link java.util.ConcurrentModificationException} if more than one entry is written to
 * the log at the same time. Any number of entries may be read via {@link BufferedLog#getEntry(long)} at any given time, but it's
 ...
 * wherein entries are permanently stored to disk. Entries are committed to the log via the {@link BufferedLog#commit(long)}
 * method. When an entry or set of entries is committed to the log, they cannot later be removed via {@link BufferedLog#truncate(long)}.
...
public class BufferedLog implements RaftLog {
  ...
  public void commit(long index) {
    ...
    int entriesPerSegment = config.getEntriesPerSegment();
    boolean compact = index / entriesPerSegment > segments.commitIndex() / entriesPerSegment;
