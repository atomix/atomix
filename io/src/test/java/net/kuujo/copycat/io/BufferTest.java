...
public abstract class BufferTest {
  ...
  public void testWriteReader() {
    Buffer readBuffer = createBuffer(16);
    readBuffer.writeLong(10);
    readBuffer.writeLong(11);
    readBuffer.flip();
    Buffer readSlice = readBuffer.slice(8, 8);
    BufferReader reader = readSlice.reader();
    Buffer writeBuffer = createBuffer(16);
    Buffer writeSlice = writeBuffer.slice(8, 8);
    writeSlice.write(reader);
    writeBuffer.position(8);
    assertEquals(writeBuffer.readLong(), 11);
  }
  ...
  public void testWriterReaderMany() {
    Buffer buffer = createBuffer(1024);
    buffer.writeInt(10).mark();
    BufferWriter writer = buffer.writer();
    assertEquals(writer.position(), 0);
    writer.writeBoolean(true).write("Hello world!".getBytes());
    byte[] bytes = new byte[12];
    BufferReader reader = buffer.reader(0);
    assertEquals(reader.position(), 0);
    assertEquals(reader.readInt(), 10);
