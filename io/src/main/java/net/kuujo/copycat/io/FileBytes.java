...
public class FileBytes extends AbstractBytes {
  ...
  public static FileBytes allocate(File file, String mode, long size) {
    return new FileBytes(file, mode, Memory.toPow2(size));
  }
   ...
   * a {@link net.kuujo.copycat.io.MappedBytes} instance.
   *
  ...
  /**
   * Maps a portion of the randomAccessFile into memory and returns a {@link net.kuujo.copycat.io.MappedBytes} instance.
   *
   * @param offset The offset from which to map the randomAccessFile into memory.
   * @param size The size of the bytes to map into memory.
  ...
  public MappedBytes map(long offset, long size, FileChannel.MapMode mode) {
    return new MappedBytes(new MappedMemoryAllocator(randomAccessFile, mode, offset).allocate(size));
  }
  ...
  public long readUnsignedInt(long offset) {
    ...
    try {
      seekToOffset(offset);
      return Integer.toUnsignedLong(randomAccessFile.readInt());
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  }
