...
public class MappedBuffer extends NativeBuffer {
  ...
  public static MappedBuffer allocate(File file, FileChannel.MapMode mode, long initialCapacity, long maxCapacity) {
    ...
    return new MappedBuffer(new MappedBytes(MappedMemory.allocate(file, mode, Memory.toPow2(initialCapacity))), 0, initialCapacity, maxCapacity);
  }
  ...
  protected Buffer createView(ReferenceManager<Buffer> referenceManager) {
    return new MappedBuffer((MappedBytes) bytes, referenceManager);
  }
