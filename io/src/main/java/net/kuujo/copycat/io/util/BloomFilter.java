 ...
 * <p>
 * The bloom filter is a probabilistic data structure that provides an interface for testing whether a value is a member
 * of a set. Members are added to the set via {@link net.kuujo.copycat.io.util.BloomFilter#add(Object)}. The existence
 * of a member in the set is tested by calling {@link net.kuujo.copycat.io.util.BloomFilter#contains(Object)}. If
 * {@link net.kuujo.copycat.io.util.BloomFilter#contains(Object)} returns {@code false} then the value is definitely not
 * a member of the set. Alternatively, if {@link net.kuujo.copycat.io.util.BloomFilter#contains(Object)} returns
 * {@code true} then that indicates only that the value <em>may</em> be a member of the set.
 * <p>
 * When members are added to the set, a series of {@link net.kuujo.copycat.io.util.HashFunction} are applied to the value.
 * The number of hashes computed is determined by the desired false positive probability provided to the
 * {@link net.kuujo.copycat.io.util.BloomFilter} constructor. The first two hashes computed are always
 * {@link net.kuujo.copycat.io.util.CityHashFunction} and {@link net.kuujo.copycat.io.util.Murmur3HashFunction}. If any
 * additional hashes are computed thereafter, those hashes will be based on the first two hashes using the following
 * algorithm:
 * <pre>
 *   {@code
 *   for (int i = 2; i < numHashes; i++) {
 *     long hash = cityHash + i * murmurHash;
 *     if (hash < 0)
 *       hash = ~hash;
 *     hashes[i] = h % bits;
 *   }
...
public class BloomFilter<T> implements AutoCloseable {
  ...
  public BloomFilter(double falsePositiveProbability, long expectedSize) {
    ...
    bits = BitArray.allocate(numBits);
  ...
  private long[] indexes(byte[] bytes, int numHashes, long bits) {
    if (numHashes == 1) {
      return new long[]{Math.abs(function1.hash64(bytes) % bits)};
    } else if (numHashes == 2) {
      return new long[]{Math.abs(function1.hash64(bytes) % bits), Math.abs(function2.hash64(bytes) % bits)};
    }

    long[] hashes = new long[numHashes];

    long h1 = function1.hash64(bytes);
    hashes[0] = Math.abs(h1 % bits);

    long h2 = function2.hash64(bytes);
