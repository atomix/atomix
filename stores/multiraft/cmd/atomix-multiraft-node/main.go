...
import (
	...
	multiraft "github.com/atomix/atomix/stores/multiraft/pkg/node"
...
func main() {
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			node.RegisterService(func(server *grpc.Server) {
				multiraftv1.RegisterNodeServer(server, multiraft.NewNodeServer(protocol))
			})
	...
	cmd.Flags().StringP("config", "c", "", "the path to the multiraft cluster configuration")
