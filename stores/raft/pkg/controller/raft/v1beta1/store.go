...
func addMultiRaftStoreController(mgr manager.Manager) error {
	...
	controller, err := controller.New("atomix-raft-store-v1beta1", mgr, options)
	if err != nil {
		return err
	}

	// Watch for changes to the storage resource and enqueue Stores that reference it
	err = controller.Watch(&source.Kind{Type: &raftv1beta1.MultiRaftStore{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}

	// Watch for changes to secondary resource MultiRaftCluster
	err = controller.Watch(&source.Kind{Type: &raftv1beta1.MultiRaftCluster{}}, &handler.EnqueueRequestForOwner{
		OwnerType:    &raftv1beta1.MultiRaftStore{},
		IsController: true,
	})
	if err != nil {
		return err
...
func (r *MultiRaftStoreReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	store := &raftv1beta1.MultiRaftStore{}
	...
	if cluster.Status.State == raftv1beta1.MultiRaftClusterNotReady &&
		store.Status.State != raftv1beta1.MultiRaftStoreNotReady {
		store.Status.State = raftv1beta1.MultiRaftStoreNotReady
	...
	if cluster.Status.State == raftv1beta1.MultiRaftClusterReady &&
		store.Status.State != raftv1beta1.MultiRaftStoreReady {
		store.Status.State = raftv1beta1.MultiRaftStoreReady
		if err := r.client.Status().Update(ctx, store); err != nil {
			return reconcile.Result{}, err
		}
...
func getProtocolConfig(partitions []raftv1beta1.RaftPartitionStatus) rsmv1.ProtocolConfig {
	var config rsmv1.ProtocolConfig
