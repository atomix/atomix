...
func (n *Protocol) Bootstrap(config raftv1.GroupConfig) error {
...
func (n *Protocol) Join(config raftv1.GroupConfig) error {
	raftConfig := n.getRaftConfig(config)
	members := make(map[uint64]dragonboat.Target)
	for _, member := range config.Members {
		members[uint64(member.MemberID)] = fmt.Sprintf("%s:%d", member.Host, member.Port)
...
func (n *Protocol) Leave(groupID raftv1.GroupID) error {
...
func (n *Protocol) newStateMachine(clusterID, nodeID uint64) dbstatemachine.IStateMachine {
	streams := newContext()
	partition := newPartition(rsmv1.PartitionID(clusterID), raftv1.MemberID(nodeID), n.host, streams)
	n.mu.Lock()
...
func (n *Protocol) getRaftConfig(config raftv1.GroupConfig) raftconfig.Config {
	...
	return raftconfig.Config{
		...
		IsObserver:         config.Role == raftv1.MemberRole_OBSERVER,
		IsWitness:          config.Role == raftv1.MemberRole_WITNESS,
	}
}
