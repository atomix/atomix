...
import (
	...
	podmemoryv1beta1 "github.com/atomix/atomix/stores/podmemory/pkg/apis/podmemory/v1beta1"
...
func addPodMemoryStoreController(mgr manager.Manager) error {
	options := controller.Options{
		Reconciler: &MultiRaftStoreReconciler{
			...
			events: mgr.GetEventRecorderFor("atomix-podmemory-storage"),
		},
		RateLimiter: workqueue.NewItemExponentialFailureRateLimiter(time.Millisecond*10, time.Second*5),
	}

	// Create a new controller
	controller, err := controller.New("atomix-podmemory-store-v1beta1", mgr, options)
	if err != nil {
		return err
	}
