...
class ServerStateMachineExecutor implements StateMachineExecutor {
  ...
  ServerStateMachineExecutor(long id, String name, String type, RaftStateMachine stateMachine, ServerContext server, ServerSessionManager sessionManager, ThreadContext stateMachineExecutor, ThreadContext snapshotExecutor) {
  ...
  private void applyCommand(long index, long sequence, long timestamp, byte[] bytes, ServerSessionContext session, CompletableFuture<OperationResult> future) {
    ...
    RaftCommand command = stateMachineExecutor.serializer().readObject(HeapBuffer.wrap(bytes));
  ...
  private void applyQuery(long index, long timestamp, ServerSessionContext session, byte[] bytes, CompletableFuture<OperationResult> future) {
    ...
    RaftQuery query = stateMachineExecutor.serializer().readObject(HeapBuffer.wrap(bytes));
