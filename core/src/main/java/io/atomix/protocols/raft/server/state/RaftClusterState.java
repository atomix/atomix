...
final class RaftClusterState implements RaftCluster, AutoCloseable {
    ...
    private final Map<RaftMember.Type, List<MemberState>> memberTypes = new HashMap<>();
        ...
        if (configuration != null) {
            ...
            for (RaftMember member : configuration.members()) {
                if (member.equals(this.member)) {
                    ...
                    MemberState state = new MemberState(new RaftMemberState(member.id(), member.type(), member.status(), updateTime), this, new SingleThreadContext(threadFactory));
    ...
    @Override
    public RaftMemberState member(NodeId id) {
     ...
     */
    public MemberState getMemberState(NodeId id) {
     ...
     * @return The member.
     */
    public RaftMemberState getRemoteMember(NodeId id) {
     ...
     */
    public List<MemberState> getRemoteMemberStates(RaftMember.Type type) {
    ...
    public CompletableFuture<Void> bootstrap(Collection<NodeId> cluster) {
        ...
        if (configuration == null) {
            if (member.type() != RaftMember.Type.ACTIVE) {
                ...
                Set<RaftMember> activeMembers = cluster.stream()
                        .filter(m -> !m.equals(member.id()))
                        .map(m -> new RaftMemberState(m, RaftMember.Type.ACTIVE, RaftMember.Status.AVAILABLE, member.updated()))
    ...
    private void join(Iterator<MemberState> iterator) {
        if (iterator.hasNext()) {
            ...
            LOGGER.debug("{} - Attempting to join via {}", member().id(), member.getMember().id());

            JoinRequest request = JoinRequest.builder()
                    .withMember(new RaftMemberState(member().id(), member().type(), member().status(), member().updated()))
                    .build();
            context.getProtocolDispatcher().join(member.getMember().id(), request).whenComplete((response, error) -> {
                ...
                if (error == null) {
                    if (response.status() == RaftResponse.Status.OK) {
                        LOGGER.info("{} - Successfully joined via {}", member().id(), member.getMember().id());
                        ...
                        LOGGER.debug("{} - Failed to join {}", member().id(), member.getMember().id());
                        resetJoinTimer();
                    } else {
        ...
        // for servers to potentially timeout and elect a leader.
            ...
            LOGGER.debug("{} - Failed to join cluster, retrying...", member.id());
            resetJoinTimer();
        }
    }
    ...
    private void cancelJoinTimer() {
        if (joinTimeout != null) {
            LOGGER.trace("{} - Cancelling join timeout", member().id());
            joinTimeout.cancel();
            joinTimeout = null;
        }
    }
    ...
    public synchronized CompletableFuture<Void> leave() {
        ...
        context.getThreadContext().execute(() -> {
            ...
            if (getActiveMemberStates().isEmpty() && configuration.index() <= context.getCommitIndex()) {
                LOGGER.trace("{} - Single member cluster. Transitioning directly to inactive.", member().id());
    ...
    private void cancelLeaveTimer() {
        if (leaveTimeout != null) {
            LOGGER.trace("{} - Cancelling leave timeout", member().id());
            leaveTimeout.cancel();
            leaveTimeout = null;
        }
    }
    ...
    RaftClusterState configure(Configuration configuration) {
        ...
        for (RaftMember member : configuration.members()) {
            if (member.equals(this.member)) {
                ...
                this.member.update(member.type(), time);
                members.add(this.member);
            } else {
                // If the member state doesn't already exist, create it.
                MemberState state = membersMap.get(member.id());
                if (state == null) {
                    state = new MemberState(new RaftMemberState(member.id(), member.type(), member.status(), time), this, new SingleThreadContext(threadFactory));
                    state.resetState(context.getLog());
                    this.members.add(state.getMember());
                    this.remoteMembers.add(state);
                    membersMap.put(member.id(), state);
    ...
    private void reassign() {
        if (member.type() == RaftMember.Type.ACTIVE && !member.equals(context.getLeader())) {
            ...
            int index = 1;
            for (MemberState member : getActiveMemberStates(Comparator.comparingInt(m -> m.getMember().hash()))) {
            ...
            List<MemberState> sortedPassiveMembers = getPassiveMemberStates(Comparator.comparingInt(m -> m.getMember().hash()));
