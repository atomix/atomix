...
import io.atomix.protocols.raft.storage.entry.CloseSessionEntry;
import io.atomix.protocols.raft.storage.entry.CommandEntry;
import io.atomix.protocols.raft.storage.entry.ConfigurationEntry;
import io.atomix.protocols.raft.storage.entry.InitializeEntry;
import io.atomix.protocols.raft.storage.entry.KeepAliveEntry;
import io.atomix.protocols.raft.storage.entry.MetadataEntry;
import io.atomix.protocols.raft.storage.entry.OpenSessionEntry;
import io.atomix.protocols.raft.storage.entry.QueryEntry;
...
final class LeaderState extends ActiveState {
  ...
  public CompletableFuture<JoinResponse> join(final JoinRequest request) {
    ...
    members.add(new RaftMemberState(member.id(), member.type(), member.status(), Instant.now()));
  ...
  public CompletableFuture<ReconfigureResponse> reconfigure(final ReconfigureRequest request) {
    ...
    RaftMemberState existingMember = context.getClusterState().member(request.member().id());
  ...
  public CompletableFuture<AppendResponse> append(final AppendRequest request) {
    ...
    if (updateTermAndLeader(request.term(), request.leader())) {
      ...
      context.setLeader(request.leader()).transition(RaftServer.State.FOLLOWER);
