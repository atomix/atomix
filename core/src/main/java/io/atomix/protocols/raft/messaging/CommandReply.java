 ...
 * {@link CommandMessage}. Command responses are sent with the
 ...
 */
public class CommandReply extends OperationReply {
  ...
  public CommandReply(Status status, RaftError error, long index, long eventIndex, Object result, long lastSequence) {
   ...
   */
  public static class Builder extends OperationReply.Builder<Builder, CommandReply> {
    @Override
    public CommandReply build() {
      validate();
      return new CommandReply(status, error, index, eventIndex, result, lastSequence);
    }
  }
}
