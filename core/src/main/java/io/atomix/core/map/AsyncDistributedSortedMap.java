...
public interface AsyncDistributedSortedMap<K extends Comparable<K>, V> extends AsyncDistributedMap<K, V> {
   ...
   */
  AsyncDistributedSortedMap<K, V> subMap(K fromKey, K toKey);
   ...
   *         bounds of the range
   */
  AsyncDistributedSortedMap<K, V> headMap(K toKey);
   ...
   *         bounds of the range
   */
  AsyncDistributedSortedMap<K, V> tailMap(K fromKey);

  @Override
  default DistributedSortedMap<K, V> sync() {
    return sync(Duration.ofMillis(DEFAULT_OPERATION_TIMEOUT_MILLIS));
  }

  @Override
  DistributedSortedMap<K, V> sync(Duration operationTimeout);
}
