    ...
    new VertxEventBusProtocolConnection(address, vertx, connectionResult -> {
      if (connectionResult.failed()) {
        ...
        context.runOnContext(v -> {
          vertx.eventBus().sendWithTimeout(this.address, new JsonObject().putString("action", "connect").putString("address", address), 5000, (Handler<AsyncResult<Message<JsonObject>>>) connectResult -> {
