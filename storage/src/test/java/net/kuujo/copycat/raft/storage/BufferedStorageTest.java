...
public class BufferedStorageTest {
  ...
  public void testReadOldEntry() {
    BufferedStorage openLog;
    try (BufferedStorage log = createLog(new LogConfig().withName(UUID.randomUUID().toString()).withEntriesPerSegment(8))) {
      openLog = log;
      checkInitial(log);
      for (int i = 0; i < 12; i++) {
        writeKeyedTestEntry(log);
  ...
  public void testCommitEntryAfterSegment() {
    BufferedStorage openLog;
    try (BufferedStorage log = createLog(new LogConfig().withName(UUID.randomUUID().toString()).withEntriesPerSegment(128))) {
      openLog = log;
      checkInitial(log);
      for (int i = 0; i < 150; i++) {
        writeKeyedTestEntry(log);
