...
import net.kuujo.copycat.raft.storage.compact.CompactionStrategy;
import net.kuujo.copycat.raft.storage.compact.RetentionPolicy;
 ...
 * user must call {@link net.kuujo.copycat.raft.storage.RaftEntry#close()} once complete or the pool will grow endlessly.
 * Calling {@link net.kuujo.copycat.raft.storage.RaftEntry#close()} on an entry created via
 * {@link BufferedStorage#createEntry()} will result in the entry being persisted to the log, while calling
 * {@link net.kuujo.copycat.raft.storage.RaftEntry#close()} on an entry acquired via {@link BufferedStorage#getEntry(long)} will result in the entry being released back
 ...
 * the size of a single segment (given the size limitations of the {@link net.kuujo.copycat.raft.storage.compact.KeyTable} and {@link OffsetIndex}), a background
 * thread will create a new segment and rewrite only active entries from each segment in order. The default
 * {@link net.kuujo.copycat.raft.storage.compact.CompactionStrategy} prioritizes compaction of recent segments over older segments
 * under the assumption that entries present in less recent segments are less likely to be duplicated.
 *
...
public class BufferedStorage implements RaftStorage {
  ...
  public static class Builder {
     ...
     * {@link net.kuujo.copycat.raft.storage.compact.LeveledCompactionStrategy} will be used.
     *
