...
func TestPrimitiveCreateClose(t *testing.T) {
	...
	err = session.CreatePrimitive(context.TODO(), runtime.PrimitiveSpec{Name: "name", Service: "service"})
	assert.NoError(t, err)
	...
	queryResponse, err := query.Run(func(conn *grpc.ClientConn, headers *protocol.QueryRequestHeaders) (*protocol.TestQueryResponse, error) {
...
func TestUnaryQuery(t *testing.T) {
	...
	response, err := query.Run(func(conn *grpc.ClientConn, headers *protocol.QueryRequestHeaders) (*protocol.TestQueryResponse, error) {
		return protocol.NewTestClient(conn).TestQuery(context.TODO(), &protocol.TestQueryRequest{
			Headers: headers,
		})
	})
