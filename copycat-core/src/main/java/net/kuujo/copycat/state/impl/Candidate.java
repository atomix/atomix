    ...
    for (RemoteMember<?> member : (Set<RemoteMember<?>>) context.cluster().remoteMembers()) {
      ...
      client.connect().whenCompleteAsync((result1, error1) -> {
        if (error1 != null) {
          ...
          client.poll(new PollRequest(context.nextCorrelationId(), context.getCurrentTerm(), context.cluster().localMember().id(), lastIndex, lastTerm)).whenCompleteAsync((result2, error2) -> {
            client.close();
            if (!complete.get()) {
