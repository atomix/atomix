 ...
 * in the cluster and opens a {@link Session}. The provided {@link #session()} will be opened once
 * the client or server has safely connected to the rest of the cluster.
 * <p>
 * The Raft implementation is designed simply to support two types of operations - {@link Command commands}
 * and {@link Query queries}. Commands {@link #submit(Command) submitted} via this interface will
...
public interface Raft {
   ...
   * <p>
   * The returned {@link Session} instance will remain constant throughout the lifetime of this
   * Raft instance. Once the instance is opened, the session will be registered with the Raft cluster and listeners
   * registered via {@link Session#onOpen(net.kuujo.copycat.Listener)} will be called. In the
   * event of a session expiration, listeners registered via {@link Session#onClose(net.kuujo.copycat.Listener)}
   * will be called.
   *
