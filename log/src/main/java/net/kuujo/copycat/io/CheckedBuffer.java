...
public class CheckedBuffer implements Buffer {
  ...
  protected CheckedBuffer(Bytes bytes) {
    ...
    this.bytes = bytes;
    this.navigator = new BufferNavigator(0, bytes.size());
  }
  ...
  @Override
  public Buffer read(byte[] bytes, long offset, int length) {
  ...
  @Override
  public Buffer write(byte[] bytes, long offset, int length) {
